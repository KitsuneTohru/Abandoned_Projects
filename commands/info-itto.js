...
for (let a = 0; a < 5; ++a) {
    ...
    embedsg2.push(new MessageEmbed()
...
const row = new MessageActionRow()
    ...
    row.addComponents(
        ...
        .setEmoji('⏩')
        .setLabel('Trang Sau')
        .setDisabled(pagesg2[id] === embeds.length - 1 )
    )
    return row
}
module.exports = {
    ...
    callback: ({user, interaction, channel}) => {
        const id = user.id
        pagesg2[id] = pagesg2[id] || 0

        const embedg2 = embedsg2[pagesg2[id]]
        let collector
        ...
        collector.on('collect', (btnInt) => {
            ...
            if(btnInt.customId === 'prev' && pagesg2[id]>0) {
                --pagesg2[id]
            } else if(
                btnInt.customId === 'next' && pagesg2[id] < embedsg2.length -1
            )   {
                ++pagesg2[id]
            }
            interaction.editReply({
                embeds: [embedsg2[pages[id]]],
                components: [getRow(id)],
            })  
        })
    },
